...
export default class PercentageCircle extends React.PureComponent {
  ...
  onCircleAnimated = ({ finished }) => {
    ...
    const callback = secondsElapsed < this.props.seconds ? this.restartAnimation : this.props.onTimeElapsed
    const updatedText = this.props.updateText(secondsElapsed, this.props.seconds)
    this.setState(
      {
